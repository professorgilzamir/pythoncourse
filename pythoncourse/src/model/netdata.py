...
def read_data(path):
    ...
    for line in fdata:
        ...
        unit_health = model.UnidadeDeSaude(ld[0], ld[1], ld[2], ld[3], ld[9], ld[10], ld[11], ender)
...
def syncdata():
    ...
    if (os.path.exists(OUTPUT_PATH) == 0):
        ...
        if content_length:
            length = int(content_length)
            model.download_length(response, out_file, length)
        else:
            demol.download(response, out_file)
    ...
    dt = model.loadlistfromcsv(RESOURCE_URL, OUTPUT_PATH, EXTRACTED_PATH)

def searchNearUnitHealth(longitude, latitude):
    print("Ainda não implementado")
    pass
def searchAllUnitHealth():
    print("Ainda não implementado")
    pass